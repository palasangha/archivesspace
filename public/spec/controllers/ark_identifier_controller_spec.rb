...
describe  ArkIdentifierController, type: :controller do
  ...
  it "should redirect to the correct URL for an accession" do
    HTTP.stub(:get_response) { ArkQueryResponseMock.new("Accession", "4", "5") }

    response = get :show, params: {:id => "1", :naan => "f00001", :ark_tag => "ark:"}

    expect(response.location).to match(/\/repositories\/5\/accessions\/4/)
  end

  it "should redirect to the correct URL for an digital object" do
    HTTP.stub(:get_response) { ArkQueryResponseMock.new("DigitalObject", "4", "5") }
